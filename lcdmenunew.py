...
def beaconstatus():
                   while 1:
                           ...
                           lcd.clear() 
                           if proc == None:
                                lcd.message("Beacon is Off \n")
                           else:
                                lcd.message("Beacon Running \n")
                           sleep(0.25)
...
def turnOn():
    global proc
    if proc is None and os.path.isfile('beacontext.txt'):
        print('Starting Beacon')
        proc = subprocess.Popen(["python", "/home/pi/lcdmenu/filetransport.py"])
        lcd.clear()
        lcd.blink(True)
        lcd.message("Beacon Starting\n")
        sleep (10)
    else:
        lcd.clear()
        lcd.message("settings missing")
        sleep(10)
...
def turnOff():
    global proc
    if proc is not None:
        print('Beacon Stopping')
        lcd.clear()
        lcd.blink(False)
        lcd.message("Beacon Stopping")
        os.remove("/home/pi/lcdmenu/beacontext.txt")
        sleep(10)
        proc.terminate()
        proc = None
...
def BMessagef():
    ...
    if word is not None:
        ...
        global bmessage
        bmessage = word
        with open("beacon.txt", "w+") as text_file:
             text_file.write(call2+" "+call2+" de "+call1+" "+call1+" "+bmessage+':''\n')  
             sleep(2)


def StartBeacon():                        
                 while  os.path.isfile('beacon.txt'):
                        lcd.message("Beaconing \n")
                        lcd.message(chr(219))
                        turnOn()
                        if lcd.is_pressed(LCD.LEFT):
                           turnOff()
...
currentLcd = lcd.set_backlight(0)
lcd.set_backlight(0)
